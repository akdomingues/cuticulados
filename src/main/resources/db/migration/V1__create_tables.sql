...
-- tabela principal de usuarios (usada por herança jpa)
...
-- tabela de profissionais (extends usuario)
...
-- tabela de clientes (extends usuario)
...
-- tabela de serviços que são oferecidos
...
-- tabela intermediadora: quais serviços cada profissional realiza (muitos para muitos)
...
-- tabela intermediadora: quais produtos são usados em cada serviço
...
-- indices para melhorar o desempenho das consultas mais frequentes, ao inves de procurar no banco todo ele procura direto no quer
create index idx_usuario_deleted_at on usuario (deleted_at);
create index idx_agendamento_cliente on agendamento (cliente_id); -- faz um select * from agendamento where cliente_id = x(tal id)
